...
namespace AutomationFramework
{
    public class Table
    {
        ...
        /// </summary>
        /// <param name="knownValue">The value to look for</param>
        /// <returns>The matching row index</returns>
        public int FindFirstRowIndexByKnownValue(string knownValue)
        {
            int i;
            i = 1;
            int r;
            ...
            while (i <= _tableHeaders.Count())
            {
                foreach (IWebElement row in _tableRows)
                {
                    ...
                    _logger.Info(row.FindElement(By.XPath(string.Format("td[{0}]", i))).Text);
                }
                r = 0;
                i++;
            }
        ...
        public bool? IsEmpty(Table w3cTable)
        {
